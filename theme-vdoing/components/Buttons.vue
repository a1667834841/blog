<template>
  <div class="buttons">
    ...
    <div
      title="极客评论"
      class="button blur go-to-comment iconfont icon-pinglun geek"
      v-show="true"
      @click="openGeekComment"
    >
      <transition name="mode">
        <!-- 弹窗内容开始 -->
        <div
          class="geek-background"
          v-show="showGeekComment"
        >
          <div
            id="div1"
            class="content"
          >
            <div id="geek-comment-head">
              <span
                id="close-button"
                @click.stop="closeGeekComment"
              >×</span>
              <h2>评论</h2>
            </div>
            <div class="geek-comment">
              <ul>
                <li>评论
                  <span>ssss</span>
                </li>
              ...
              <ul>
                <li>评论
                  <span>ssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss</span>
                </li>
                <li>回复</li>
              </ul>
              <ul>
...
import geekArticles from "../api/geek-articles"; // 本地存储
...
export default {
  ...
  methods: {
    ...
    openGeekComment() {
      console.log("打开评论");
      this.showGeekComment = true;
      let articleTitle = storage.get("articleTitle");
      ...
      var articleForm = { aid: 248880, prev: 0 };
      // 根据文章名读取评论
      // var articels = geekArticles.getArticles(formData);
      var articels = geekArticles.getComments(articleForm);
      console.log(articels);
    },
    closeGeekComment() {
      console.log("关闭评论");
      this.showGeekComment = false;
    },
  },
  watch: {
    "$route.path"() {
